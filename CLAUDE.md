...
- **Map** (`MapTab`): Phaser **isometric** map (`MapScene.ts`) — every tile is a 2:1 iso cube PILLAR. The **plains
  ...
  **Dangerosité des packs** : plus de label ×count — la tuile d'un pack est **surlignée** (losange plein
  `hl-fill` sous la créature) teinté du **jaune au rouge** selon la taille du pack (`dangerTint`,
  `DANGER_MAX = 8` = rouge plein). **Fond ciel** : le canvas Phaser est **transparent** (`transparent: true`,
