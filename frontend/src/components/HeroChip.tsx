  ...
  return (
    <div className={`mhb-chip ${layout === "column" ? "col" : ""} ${selected ? "sel" : ""} ${dead ? "dead" : ""}`}>
      <button
        className="mhb-pick"
        disabled={dead}
        title={dead ? t("{name} est à terre", { name: hero.name }) : t("Sélectionner {name}", { name: hero.name })}
        onClick={onSelect}
      >
      ...
      {onOpenSheet && (
        <button className="mhb-sheet" title={t("Fiche du personnage")} onClick={onOpenSheet}>
          ⓘ
        </button>
      )}
    </div>
